...
export default function PointersPage() {
  ...
  // Step-4 ephemeral state — reset on every chunk advance
  ...
  useEffect(() => {
    if (!isLoading) seedChunks(CHUNK_IDS);
  }, [isLoading, seedChunks]);

  // ── Derived ───────────────────────────────────────────────────────────────
  const chunk        = CHUNKS[currentChunkIndex];
  const isLastChunk  = currentChunkIndex === TOTAL - 1;

  // Fresh card data — only populated after a grade is committed so the
  // next_review_date shown reflects the review we just recorded.
  const committedCard = gradeCommitted ? getCard(chunk.id) : null;

  // ── Navigation ────────────────────────────────────────────────────────────

  /** Advance to the next chunk, or mark the lesson done if at the end. */
  ...
  // ActiveRecallBlock fires onComplete only when every blank is correct → 100 %
  const handleRecallComplete = useCallback(() => {
    setStep(4);
  }, []);

  /** Persist the SM-2 grade; show confirmation — user then clicks Next manually. */
      ...
      <div className="min-h-screen bg-slate-950 flex items-center justify-center px-6">
        <div className="text-center space-y-5 max-w-sm w-full">
          ...
          <h2 className="text-2xl font-bold text-slate-100">
            Hoàn thành toàn bộ học phần!
          </h2>
          <p className="text-slate-400 font-mono text-sm">
            Đã hoàn thành {TOTAL} chunk · Kết quả đã lưu vào SRS
          </p>

          {/* Per-chunk SRS summary */}
            ...
            <div className="space-y-1">
              {CHUNKS.map((c) => {
                ...
                return (
                  ...
                  >
                    <span className="font-mono text-xs text-slate-500 flex-shrink-0 w-14">
                      {c.id}
                    </span>
          ...
          <button
            ...
            className="w-full px-6 py-2.5 rounded-lg border border-slate-700
    ...
    <div className="min-h-screen bg-slate-950 text-slate-100">
      ...
      <div className="relative z-10 max-w-2xl mx-auto px-5 pt-10 pb-24">
            ...
            <div className="flex items-center gap-1.5">
              {CHUNKS.map((c, i) => (
                <span
                  ...
                  aria-label={`${c.id}${i < currentChunkIndex ? ' — xong' : i === currentChunkIndex ? ' — đang học' : ''}`}
                />
              ))}
            </div>
        ...
        >
              ...
              {step === 1 && (
                <div className="mt-6 flex justify-end">
                  <button
                    type="button"
                    onClick={() =>
                      setStep(chunk.feynman_prompt ? 2 : chunk.code_snippet ? 3 : 4)
                    }
          ...
          {/* ── Step 4: SRS Self-rating + Next Chunk ────────────── */}
          {step === 4 && (
            <section aria-label="Đánh giá SRS và chuyển chunk">
              ...
              <p className="text-sm text-slate-300 leading-relaxed mb-5">
                ...
                Điểm số quyết định khi nào hệ thống nhắc bạn ôn lại.
              </p>

              {/* Grade picker — shown until committed */}
              {!gradeCommitted && (
                <>
                  <div
                    className="grid grid-cols-2 sm:grid-cols-3 gap-2"
                    role="group"
                    aria-label="Chọn mức độ ghi nhớ (0–5)"
                  >
                  ...
                  <div className="flex items-center justify-between mt-4">
                    <p className="text-xs text-slate-600 font-mono">
                      {pendingGrade === null
                        ? 'Chọn một mức độ bên trên'
                        : `${GRADE_META[pendingGrade].label} — ${pendingGrade * 20}%`}
                    </p>
                    <button
                      type="button"
                      disabled={pendingGrade === null}
                      onClick={() => {
                        if (pendingGrade !== null) handleCommitGrade(pendingGrade);
                      }}
                      className={[
              ...
              {/* Confirmation + next-chunk CTA — shown after commit */}
              {gradeCommitted && pendingGrade !== null && (
                <div className="space-y-4">

                  {/* SRS result card */}
                  <div
                    className="rounded-lg border border-emerald-500/30 bg-emerald-500/5 px-5 py-4 space-y-1.5"
                    role="status"
                    aria-live="polite"
                  >
                    <div className="flex items-center gap-2">
                      ...
                      <p className="text-sm font-semibold text-emerald-300">
                        Đã lưu — Điểm SM-2:{' '}
                        <span className="font-mono">{pendingGrade}/5</span>
                  ...
                  {/* Next Chunk / Last-chunk CTA */}
                  {isLastChunk ? (
                    /* ── Last chunk: complete + exam entry ──────────── */
                    <div className="flex flex-col gap-3">
                      {/* Complete lesson (navigates to done screen) */}
                      <button
                        type="button"
                        onClick={goNextChunk}
                      ...
                      <button
                        ...
                        aria-label="Vào phòng thi thực chiến — code C++ với Judge0"
                      >
